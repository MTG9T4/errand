...
This is **Errand**, a TypeScript framework for building stateful AI agents.
...
- **`packages/*`** - Extensions for platforms (discord, twitter, telegram),
  storage (supabase, chroma, mongo), chains (hyperliquid, finance), utilities
  (cli, create-errand, synthetic)
...
When you create an agent with `createErrand()`:

```
createErrand(config) → Agent instance
