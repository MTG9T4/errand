...
Errand and x402 payment middleware. Users pay $0.1 per request to interact
...
```
Client Request → x402 Payment → Errand Agent → Response
                     ↓
                  Payment
                 Processing
```
...
```bash
# Deploy with default settings (errand-labs-staging project)
bun run deploy
...
3. Your service will be available at:
   - `https://ai-assistant.agent.errand.systems`
...
```bash
# Install the Errand deploy CLI
pnpm add -g @errand/deploy

# Deploy the service
errand-deploy deploy \
  --name ai-assistant \
...
```bash
# Health check (free)
curl https://ai-assistant.agent.errand.systems/health

# Service info (free)
curl https://ai-assistant.agent.errand.systems/

# AI request with payment (using the client)
bun run client.ts --url https://ai-assistant.agent.errand.systems
```
...
```bash
# View logs
errand-deploy logs ai-assistant --project your-project

# Follow logs in real-time
errand-deploy logs ai-assistant --project your-project --follow

# List all deployments
errand-deploy list --project your-project
```
...
# Remove deployment
errand-deploy delete ai-assistant --project your-project
```
