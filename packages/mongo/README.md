...
A MongoDB integration package for the ErrandAI platform, providing persistent key-value storage capabilities.
...
- **Memory System Integration**: Works seamlessly with the ErrandAI memory system
...
```bash
npm install @errand/mongo
# or
yarn add @errand/mongo
# or
pnpm add @errand/mongo
```
...
```typescript
import { createMongoMemory } from "@errand/mongo";
import { createErrand } from "@errand/core";

// Create the memory system
const memory = createMongoMemory({
  uri: process.env.MONGODB_URI!,
  dbName: "errand_memory",        // Optional: defaults to "errand_memory"
...
```typescript
import { createMongoKVProvider } from "@errand/mongo";
...
```typescript
// Old API (deprecated)
import { createMongoMemoryStore } from "@errand/mongo";
...
- `@errand/supabase` - Full persistence (KV + Vector + Graph)
- `@errand/chroma` - Vector storage only
...
See the main ErrandAI repository for contribution guidelines.

## License

MIT - See the main ErrandAI repository for license details.