...
A Supabase integration package for the ErrandAI platform, providing comprehensive memory storage capabilities including key-value storage, vector embeddings, and graph relationships.

## Features

- **Complete Memory System**: Implements the full ErrandAI memory interface
...
```bash
npm install @errand/supabase
# or
yarn add @errand/supabase
# or
pnpm add @errand/supabase
```
...
```typescript
import { createSupabaseMemory } from "@errand/supabase";
...
```typescript
// Old API
import { createSupabaseBaseMemory } from "@errand/supabase";
