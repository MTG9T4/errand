...
Firebase Firestore integration for the ErrandAI memory system. This package provides persistent key-value storage using Firebase Firestore while using in-memory providers for vector and graph operations.

## Installation

```bash
pnpm add @errand/firebase firebase-admin
```
...
```typescript
import { createFirebaseMemory } from "@errand/firebase";
...
const memory = createFirebaseMemory({
  ...
  collectionName: "errand_kv", // optional, defaults to "kv_store"
});
...
```typescript
import { createFirebaseKVProvider } from "@errand/firebase";
...
```typescript
// Old API (deprecated)
import { createFirebaseMemoryStore } from "@errand/firebase";
