...
ChromaDB integration for the ErrandAI memory system. This package provides persistent vector storage using ChromaDB while using in-memory providers for key-value and graph operations.

## Installation

```bash
pnpm add @errand/chroma chromadb
```
...
import { createChromaMemory } from "@errand/chroma";

// Create memory system
const memory = createChromaMemory({
  path: "http://localhost:8000", // ChromaDB server URL
  collectionName: "my_agents", // optional, defaults to "errand_vectors"
});
...
interface ChromaMemoryConfig {
  ...
  path?: string;
  /** Collection name (default: "errand_vectors") */
...
```typescript
import { createChromaVectorProvider } from "@errand/chroma";
...
CHROMA_COLLECTION="errand_vectors"      # Default collection name
...
```typescript
// Old API (deprecated)
import { createChromaVectorStore } from "@errand/chroma";
