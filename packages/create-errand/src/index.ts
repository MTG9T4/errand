...
const program = new Command()
  .name("create-agent")
  .description(
    "🤖 Bootstrap a new Errand AI agent with extensions and model providers"
  )
  .version("0.1.0")
  .argument(
    "[directory]",
...
export async function main(
  ...
  console.log();
  console.log(chalk.bold("🚀 Creating a new Errand agent..."));
  console.log();
  ...
  for (const ext of selectedExtensions) {
    if (ext === "cli") {
      extensionImports.push(`import { cliExtension } from "@errand/cli";`);
      extensionsList.push("cliExtension");
    } else if (ext === "twitter") {
      extensionImports.push(`import { twitter } from "@errand/twitter";`);
      extensionsList.push("twitter");
    } else if (ext === "discord") {
      extensionImports.push(`import { discord } from "@errand/discord";`);
  ...
  console.log();
  console.log(
    chalk.green.bold("🎉 Your Errand agent has been created successfully!")
  );
  console.log();
