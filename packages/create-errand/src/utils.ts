...
export async function getLatestDependencies(
  ...
  if (selectedExtensions.includes("cli")) {
    extensionDependencies["@errand/cli"] = "^0.2.13";
  }
  if (selectedExtensions.includes("twitter")) {
    extensionDependencies["@errand/twitter"] = "^0.2.13";
  }
  if (selectedExtensions.includes("discord")) {
    extensionDependencies["@errand/discord"] = "^0.2.13";
    extensionDependencies["discord.js"] = "^14.17.3";
  }
  if (selectedExtensions.includes("telegram")) {
    extensionDependencies["@errand/telegram"] = "^0.2.13";
...
export function generateTemplateContent(
  ...
  // Replace extensions list in createErrand if specified
  ...
  // Add header comment
  const headerComment = `/**
 * Errand agent with ${extensionsList.join(", ")} extension(s)
...
export function createEnvVariables(
  ...
  const envVariables = ["# Errand Environment Variables\n"];
...
A Errand agent with the following extensions:
