...
import { createServiceManager } from "./runtime/service-provider";
import { TaskRunner } from "./market/task";
import {
  getContextId,
  createContextState,
...
import { randomUUIDv7, tryAsync } from "./io/utils";
import { promptTemplate } from "./mind/prompts/main";
import { defaultPromptBuilder } from "./mind/prompts/default-builder";
import { defaultXmlResponseAdapter } from "./io/response/default-xml-adapter";
import type { DeferredPromise } from "p-defer";
import { runAgentContext } from "./market/tasks";
import { SimpleTracker } from "./io/utils";

/**
 * Creates and configures a new Errand AI agent instance
 *
 * This is the main factory function for creating an Errand agent with the specified
 ...
 * ```
 */
export function createErrand<TContext extends AnyContext = AnyContext>(
  ...
  // Log agent creation
  logger.info("agent:create", "Creating Errand agent", {
    model: model,
  ...
  const agent: Agent<TContext> = {
    ...
    async start(args) {
      ...
      logger.info("agent:start", "Starting Errand agent", {
        args,
        booted,
