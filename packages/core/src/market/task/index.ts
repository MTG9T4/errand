...
import type { Logger } from "../../io/logger";
...
type TaskOptions = {
   ...
   * Actual delay for retry N is `base * 2^(N-1)` capped by `retryMaxMs`.
   * Defaults to 250ms.
   */
  retryBaseMs?: number;
  /** Hard cap for the backoff delay. Defaults to 30s. */
  retryMaxMs?: number;
  /**
   * Add up to 25% random jitter to each backoff delay so a fleet of
   * runners doesn't retry in lockstep. Defaults to true.
   */
  retryJitter?: boolean;
  priority?: number;
  queueKey?: string;
  ...
  private async processTask(instance: TaskInstance) {
    while (true) {
      ...
      if (instance.attempts > 1) {
        // Self-healing: exponential backoff with jitter between attempts, so
        // a flapping downstream (model API, RPC, MCP server) gets breathing
        // room to recover instead of being hammered on a fixed cadence.
        const baseMs = instance.options.retryBaseMs ?? 250;
        const maxMs = instance.options.retryMaxMs ?? 30_000;
        const backoffMs = Math.min(
          baseMs * 2 ** (instance.attempts - 2),
          maxMs
        );
        const jitterMs =
          instance.options.retryJitter === false
            ? 0
            : Math.random() * backoffMs * 0.25;
        const delayMs = Math.min(backoffMs + jitterMs, maxMs);
        this.logger.debug("task:retry", "Backing off before retry attempt", {
          taskId: instance.id,
          taskKey: instance.task.key,
