...
description: Common patterns for working with actions, inputs, and outputs in Errand agents.
---
...
import { createErrand, context, action, input, output } from "@errand/core";
...
// Complete agent
const ecommerceAgent = createErrand({
  model: openai("gpt-4o"),
...
The power of Errand comes from how seamlessly these building blocks integrate to create sophisticated, stateful agent behaviors.