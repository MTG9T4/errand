...
description: The core components that make up a Errand agent.
---

Every Errand agent is built from four main building blocks that work together to create intelligent, stateful behavior. These components handle how agents receive information, maintain state, perform actions, and respond to users.
...
import { output } from "@errand/core";
import * as z from "zod";
...
import { action } from "@errand/core";
import * as z from "zod";
...
const agent = createErrand({ actions: [globalTimeAction] });
