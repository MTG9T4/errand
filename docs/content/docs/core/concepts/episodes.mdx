---
title: Episodes
description: How Errand captures, stores, indexes, and retrieves conversational episodes, plus how to customize episode boundaries and metadata.
---
...
Errand collects logs from working memory into episodes, stores them in key–value storage, and indexes summaries (and optionally the logs) into vector memory for retrieval.

## When Episodes Form

By default, Errand starts an episode when a meaningful interaction begins and ends it once the agent responds or completes a significant step.
