...
description: Structure, parsing, and customization of Errand prompts and XML responses.
---

## Overview

Context engineering is how Errand shapes what the model sees and how it should respond. It covers:
...
- `handlers/handle-stream.ts` uses `xmlStreamParser(...)` to parse tags as they arrive and converts them into Errand logs:
...
The response parser recognizes these tags by default and maps them to Errand logs:
...
import type { PromptBuilder } from "@errand/core";
import { mainPrompt } from "@errand/core";
...
const agent = createErrand({ prompt: myPrompt });
```
