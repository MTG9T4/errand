---
title: Quickstart
description: Get started with Errand Router in under 5 minutes
---

# Quickstart Guide

Make your first API call to the Errand Router in under 5 minutes.
...
Get your key from [router.errand.systems](https://router.errand.systems).
...
```javascript
import { generateX402Payment } from "@errand/ai-sdk-provider";
...
const response = await fetch(
  "https://router.errand.systems/v1/chat/completions",
  {
    method: "POST",
    headers: {
...
```javascript
import { generateX402PaymentBrowser } from "@errand/ai-sdk-provider";
...
```bash
curl -X POST https://router.errand.systems/v1/chat/completions \
...
Errand Router standardizes all responses to the OpenAI format, regardless of the
underlying provider:
...
Errand Router supports models from multiple providers including OpenAI,
...
curl -H "Authorization: Bearer YOUR_API_KEY" \
  https://api-beta.errand.systems/v1/models
```
...
client = OpenAI(
    ...
    base_url="https://router.errand.systems/v1"
)
...
const client = new OpenAI({
  apiKey: "YOUR_API_KEY",
  baseURL: "https://router.errand.systems/v1",
});
...
### Errand SDK Integration

For TypeScript projects using Vercel AI SDK, see the
[Errand SDK guide](./errand-sdk) for detailed integration instructions with
payment support.
...
- [Errand SDK Integration](./errand-sdk) - TypeScript integration with Vercel AI
  SDK
...
- [Examples](https://github.com/MTG9T4/errand/tree/main/examples) -
