...
description:
  Build a paid AI assistant API using Errand agents and x402 micropayments
---

This tutorial shows you how to create an AI nanoservice - a pay-per-use API
endpoint where users pay micropayments for each AI request. We'll use Errand
...
```bash
bun add @errand/core @ai-sdk/openai hono @hono/node-server x402-hono dotenv zod
```
...
Contexts in Errand manage conversation state and memory. Add this to your
`server.ts`:
...
Initialize the Errand agent with your context:
...
[examples/x402/nanoservice](https://github.com/MTG9T4/errand/tree/main/examples/x402/nanoservice)
